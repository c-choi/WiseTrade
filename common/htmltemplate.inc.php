...
class tag_val extends DataTag{
	...
	var $tostring="<?php @print \$this->null2br(\$val%1\$s); ?>\n";
}

class tag_rval extends DataTag{
	var $matchregexp='/\{rval ([^\}]+)\}/i';
	var $fromstring="{rval %s}";
	var $tostring="<?php @print \$this->null2br(htmlspecialchars(\$val%1\$s)); ?>\n";
}

class tag_hval extends DataTag{
	var $matchregexp='/\{hval ([^\}]+)\}/i';
	var $fromstring="{hval %s}";
	var $tostring="<?php @print htmlspecialchars(\$val%1\$s); ?>\n";
}

class tag_pval extends DataTag{
	var $matchregexp='/\{pval ([^\}]+)\}/i';
	var $fromstring="{pval %s}";
	var $tostring="<?php @print \$val%1\$s; ?>\n";
}
class tag_halfval extends DataTag{
	var $matchregexp='/\{halfval ([^\}]+)\}/i';
	var $fromstring="{halfval %s}";
	var $tostring="<?php @print floor((\$val%1\$s) / 2); ?>\n";
}
...
class StandardParser extends TemplateParser{
	function StandardParser(){
		...
		$this->add(new tag_hval());
		$this->add(new tag_pval());
		$this->add(new tag_halfval());
