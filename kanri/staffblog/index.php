<?php
	...
	function add_button($data){
		...
		if (is_uploaded_file($_FILES['image_file']['tmp_name'])){
			...
			if($format == "jpeg"){
				imagejpeg($imageNew, UPLOAD_STAFF_IMAGE . $_FILES['image_file']['name']);
			}elseif($format == "gif"){
				imagegif($imageNew, UPLOAD_STAFF_IMAGE . $_FILES['image_file']['name']);
			}elseif($format == "png"){
				imagepng($imageNew, UPLOAD_STAFF_IMAGE . $_FILES['image_file']['name']);
			}

			chmod(UPLOAD_STAFF_IMAGE . $_FILES['image_file']['name'], 0666);
			$_POST['image_name'] = $_FILES['image_file']['name'];
		}
		

		if(!($fp = fopen(STAFF_CSV, 'w'))){
			die('csvファイルを読み込めません');
		}
	...
	function gc_image($data){
		...
		$drc = dir(UPLOAD_STAFF_IMAGE);

		while($fl = $drc->read()){
			$lfl = UPLOAD_STAFF_IMAGE . $fl;
			$din = pathinfo($lfl);

			if(!is_dir($lfl) && ($fl!=".." && $fl!=".") && $fl!=NO_IMAGE_STAFF){
...
<title>社員ブログ管理画面</title>
<meta name="keywords" content="社員ブログ管理画面">
<meta name="Description" content="社員ブログ管理画面">
...
<?php
   ...
   $body = 
    ...
    <h3>' . "$array[0]" . '</h3>
    <h4>' . "$array[1]" . '</h4>
    <h5 class="imgright"><img src="' . UPLOAD_STAFF_IMAGE . "$array[3]" . '" alt="" width="240" height="180"></h5>
    <p>' . "$array[2]" . '</p>
